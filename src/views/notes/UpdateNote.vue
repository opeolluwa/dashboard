<template>
    <form @submit.prevent="updateEntry">
        <BaseTextInput label="title" type="text" placeholder="Ex: how to create repo" v-model="note.title"
            class="field" />
        <!--text editor here -->
        <BaseTextarea v-if="!useMdEditor" placeholder="provide note details here" label="content"
...
export default defineComponent({
    ...
    methods: {
        ...
        //update note entry
        async updateEntry() {
            const successfulRequest = await this.editNote(String(this.noteId), { ...this.note });
