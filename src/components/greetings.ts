...
if ((Number(hour) >= 6) && (Number(hour) <= 8)) { _g.time = _r(options.morning); }
//if between 8:00AM & 10:00AM
else if (hour >= 8 && hour <= 10) { _g.time = _r(options.day_break); }
//if afternoon 12:00PM - 3:00PM
else if (hour >= 12 && hour <= 15) { _g.time = _r(options.afternoon); }
//if evening : 4:00PM - 8:00PM
else if (hour >= 16 && hour <= 20) { _g.time = _r(options.evening); }
//if midnight between 1:00AM - 3:00AM
else if (hour >= 1 && hour <= 3) { _g.time = _r(options.midnignt); }
else {
    _g.time = "";

}
// HANDLE SPECIAL OCCASIONS
_g.occasion = (new_month) ? _r(options.new_month) :
    //christmas
...
message = _g.occasion || _g.time || _g.day || _g.other;
console.log(message);
